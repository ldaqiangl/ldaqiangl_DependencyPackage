...
@implementation UIImage (Effect_DQExt)

#pragma mark - Public
/** 根据色值改变图片的暗度 */
- (UIImage *)darkToValue_DQExt:(float)darkValue
{
    return [self pixelOperationBlock_DQExt:^(UInt8 *redRef,
                                             UInt8 *greenRef,
                                             UInt8 *blueRef)
            {
...
#pragma mark - Private
- (UIImage *)pixelOperationBlock_DQExt:(void(^)(UInt8 *redRef,
                                          UInt8 *greenRef,
                                          UInt8 *blueRef))block
{
    if(block == nil)
